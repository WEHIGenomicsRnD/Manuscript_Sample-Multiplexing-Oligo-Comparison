...
From the **3A_qc_dimred** notebook. The number of reads was downsampled to the the same as 10x Genomics v3.1 fresh nuclei.
...
Cluster 4 and 5 look the most different and are the ones spaced most apart in the UMAP
...
There are no markers that discriminate cluster 2 and 3. I think they are the same cell type.

```{r expression, fig.asp=1}
...
```{r umap_ki67, fig.asp=1}
umap_ki67 <- plotReducedDim(sce, dimred="UMAP", colour_by="MKI67") + theme_Publication(base_size = 18)
...
```{r umap_nrx1}
umap_nrx1 <- plotReducedDim(sce, dimred="UMAP", colour_by="NRXN1") + theme_Publication(base_size=22)
...
saveRDS(sce, here::here(
  "data/genomics_data/SCEs", "PDX", 
  "parse_downsample_anno-sce.rds"
))
```

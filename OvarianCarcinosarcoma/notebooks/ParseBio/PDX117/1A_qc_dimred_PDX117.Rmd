...
```{r saveSCE}
sce <- readRDS("/vast/scratch/users/brown.d/Parse_PDX_Zac/SCEs/pdx105_117.PARSE.recodeSNPdonor.rds")

sce <- sce[,sce$SNP_donor == "donor1"]
```

Recap in the existing UMAP which I will later overrwrite.  
There are cells that group with PDX105 on the left of the UMAP. Remove those as they are clearly doublets with the other sample.

There still could be homotypic doublets.
...
dim(sce[,reducedDim(sce, "UMAP")[,1] > 0])
sce <- sce[,reducedDim(sce, "UMAP")[,1] > 0]

```

Replot after removing the cell doublets

```{r}
plotReducedDim(sce, dimred="UMAP", colour_by="sum") 
```

---
title: "CRISPRclean gene correlation and differential expression PDX"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
knitr::opts_chunk$set(echo = TRUE,
                      ...
                      dpi = 300,
                      fig.asp=0.6, 
                      fig.path='./figures/correlation_DE/',
...
Thee correlation here is poor here reflecting the CRISPRclean destruction has worked very well.  
I would expect mitochondrial filtering to work poorly here on the CRISPR sample.
...
gene_tb$Symbol <- gsub("GRCh38_", "", gene_tb$Symbol)
...
Sum of counts per gene. Good correlation. The depletion worked less than PBMCs but the expression of genes in the panel is much lower.

Some targets are not hit but need a closer look.

```{r gene_gene_sum}
plt1 <- ggplot(gene_tb_wide, 
             aes(x = Undepleted, y= CRISPRClean, colour = Category)) + 
...
non_specific %>% 
  select(Symbol.x:Category,DepletePercent) %>% 
  head(20)
```

## Look at the expression of genes in the panel to check the ones that didn't work

Mostly non-variable genes that at a glance do not look like they encode any important function.

```{r}
failed_panel <- gene_tb_wide %>% 
  arrange(desc(DepletePercent)) %>% 
  filter(CRISPR_panel == TRUE) %>% 
  filter(Undepleted > 100) %>% 
  filter(CRISPRClean > 100) %>% 
  filter(DepletePercent > 0)
...
write_csv(gene_tb, here::here(
  "CRISPRclean/data/PDX/Matrices", "PDX_gene_metrics.csv.gz"
))

write_csv(non_specific, here::here(
  "CRISPRclean/data/PDX/Matrices", "PDX_nonSpecific_deplete.csv.gz"
))
```

# Conclusion

The Jumpcode depletion worked mostly well but there are a few transcripts that weren't hit.
